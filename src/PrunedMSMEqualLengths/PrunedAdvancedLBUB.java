...
public class PrunedAdvancedLBUB {
     ...
     */
    public PrunedAdvancedLBUB(double c, double[] X, double[] Y, double constant, String UB) {
        this.c = c;
        this.X = X;
        this.Y = Y;
        this.m = X.length;
        ...
        this.precomputeRemainingCostArray();
    }

    public void precomputeRemainingCostArray() {
        this.remainingCostArray = new double[m + 1][m + 1];

        for (int i = 2; i < m; i++) {
            for (int j = 2; j < m; j++) {
                double dist = Math.max(Math.abs(ts1[i] - this.constant + Math.abs(ts2[j]) - this.constant) - c, 0);
                this.remainingCostArray[i][j] = Math.max(Math.abs(remainingCostX[i]) - remainingCostY[j] - (((m - i) - (m - j)) * this.c) - dist, 0) - 0.000001;
            }
        }
        for (int i = 1; i < m; i++) {
            double dist = Math.abs(ts1[i] - this.constant) + Math.abs(ts2[m] - this.constant);
            this.remainingCostArray[i][m] = Math.max(Math.abs(remainingCostX[i]) - remainingCostY[m] - (((m - i)) * this.c) - dist, 0) - 0.000001;
            dist = Math.abs(ts1[m] - this.constant) + Math.abs(ts2[i] - this.constant);
            this.remainingCostArray[m][i] = Math.max(Math.abs(remainingCostX[m]) - remainingCostY[i] - ((-(m - i)) * this.c) - dist, 0) - 0.000001;
        }
        this.remainingCostArray[1][1] = Math.max(Math.abs(remainingCostX[1] - remainingCostY[1]) * this.c - Math.abs(ts1[1] - this.constant) + Math.abs(ts2[1] - this.constant), 0) - 0.000001;
        this.remainingCostArray[m][m] = 0;


        for (double[] arr : remainingCostArray) {
    ...
    public double[][] lowerBoundsPerEntry() {
        ...
        for (int i = 0; i < this.m + 1; i++) {
            for (int j = 0; j < this.m + 1; j++) {
                lowerBounds[i][j] = Math.round(this.getLowerBound(i, j, ts1[i], remainingCostX[i]) * 100) / 100.;
            }
        }
        return lowerBounds;
    }
